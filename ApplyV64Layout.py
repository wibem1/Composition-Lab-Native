...
# Keep all non-Main helpers after the V6.3 action bar/import helpers.  We replace
# only the visible Main builder and its V6.3 UI helper block.
new = r'''    private func buildCompositionWorkspace(_ parent: NSView) {
        // V6.4 follows the approved MusicChat mock-up: generous outer margins,
        // compact controls, chat + idea side by side, ten real cards, transport last.
        ...
        let controls = buildV64Controls()
        root.addArrangedSubview(controls)
        controls.widthAnchor.constraint(equalTo: root.widthAnchor).isActive = true

        let work = buildV64WorkArea()
        root.addArrangedSubview(work)
        work.widthAnchor.constraint(equalTo: root.widthAnchor).isActive = true
        work.setContentHuggingPriority(.defaultLow, for: .vertical)
        work.setContentCompressionResistancePriority(.defaultLow, for: .vertical)

        let deck = buildV64Deck()
        root.addArrangedSubview(deck)
        deck.widthAnchor.constraint(equalTo: root.widthAnchor).isActive = true

        let bottom = buildV64BottomBar()
        root.addArrangedSubview(bottom)
        bottom.widthAnchor.constraint(equalTo: root.widthAnchor).isActive = true
    }
    ...
    private func v64Panel() -> NSBox {
        ...
        b.cornerRadius = 12
        b.borderWidth = 1
        b.contentViewMargins = NSSize(width: 14, height: 12)
        return b
    }
    ...
    private func buildV64Controls() -> NSView {
        ...
        let ai = NSTextField(labelWithString: "KI")
        ai.font = .systemFont(ofSize: 12, weight: .semibold)
        row.addArrangedSubview(ai)
        row.addArrangedSubview(providerPop)
        row.addArrangedSubview(modelPop)
        row.addArrangedSubview(effortPop)
        row.addArrangedSubview(NSView())

        func label(_ text: String) { row.addArrangedSubview(NSTextField(labelWithString: text)) }
        func field(_ f: NSTextField, _ placeholder: String, _ width: CGFloat) {
            f.placeholderString = placeholder
            f.widthAnchor.constraint(equalToConstant: width).isActive = true
            row.addArrangedSubview(f)
        }
        label("Takte"); field(measuresField, "24", 54)
        field(meterField, "4/4", 54)
        label("Tempo"); field(tempoField, "96", 58)
        label("Tonart"); field(musicalKeyField, "C-Dur", 86)
        label("Instrumente")
        ensembleField.placeholderString = "Piano"
        ensembleField.widthAnchor.constraint(equalToConstant: 150).isActive = true
        row.addArrangedSubview(ensembleField)
        return box
    }
    ...
    private func buildV64WorkArea() -> NSView {
        ...
        cs.spacing = 10
        chat.contentView = cs
        let title = NSTextField(labelWithString: "MusicChat")
        title.font = .systemFont(ofSize: 20, weight: .bold)
        cs.addArrangedSubview(title)

        promptView.font = .systemFont(ofSize: 13)
        let promptScroll = textScroll(promptView, minHeight: 58)
        promptScroll.heightAnchor.constraint(equalToConstant: 66).isActive = true
        cs.addArrangedSubview(promptScroll)
        promptScroll.widthAnchor.constraint(equalTo: cs.widthAnchor).isActive = true
        ...
        cs.addArrangedSubview(log)
        log.widthAnchor.constraint(equalTo: cs.widthAnchor).isActive = true
        log.heightAnchor.constraint(greaterThanOrEqualToConstant: 180).isActive = true
        ...
        input.addArrangedSubview(chatInput)
        let send = NSButton(title: "Senden", target: self, action: #selector(chatPressed))
        send.bezelStyle = .rounded
        send.widthAnchor.constraint(equalToConstant: 92).isActive = true
        input.addArrangedSubview(send)
        cs.addArrangedSubview(input)
        input.widthAnchor.constraint(equalTo: cs.widthAnchor).isActive = true

        let idea = v64Panel()
        let isv = NSStackView()
        isv.orientation = .vertical
        isv.alignment = .leading
        isv.spacing = 9
        idea.contentView = isv
        let it = NSTextField(labelWithString: "Aktuelle Kompositionsidee")
        it.font = .systemFont(ofSize: 16, weight: .bold)
        isv.addArrangedSubview(it)
        conceptView.isEditable = false
        conceptView.isSelectable = true
        conceptView.font = .systemFont(ofSize: 13)
        let ideaScroll = textScroll(conceptView, minHeight: 120)
        ideaScroll.heightAnchor.constraint(equalToConstant: 140).isActive = true
        isv.addArrangedSubview(ideaScroll)
        ideaScroll.widthAnchor.constraint(equalTo: isv.widthAnchor).isActive = true
        let noteLabel = NSTextField(labelWithString: "Notizen / musikalischer Impuls")
        noteLabel.textColor = .secondaryLabelColor
        isv.addArrangedSubview(noteLabel)
        resultLabel.maximumNumberOfLines = 5
        resultLabel.lineBreakMode = .byWordWrapping
        resultLabel.font = .systemFont(ofSize: 12)
        isv.addArrangedSubview(resultLabel)
        resultLabel.widthAnchor.constraint(equalTo: isv.widthAnchor).isActive = true

        row.addArrangedSubview(chat)
        row.addArrangedSubview(idea)
        chat.widthAnchor.constraint(equalTo: row.widthAnchor, multiplier: 0.66, constant: -7).isActive = true
        idea.widthAnchor.constraint(equalTo: row.widthAnchor, multiplier: 0.34, constant: -7).isActive = true
        chat.heightAnchor.constraint(greaterThanOrEqualToConstant: 340).isActive = true
        idea.heightAnchor.constraint(equalTo: chat.heightAnchor).isActive = true
        return row
    }
    ...
    private func buildV64Deck() -> NSView {
        ...
        scroll.heightAnchor.constraint(equalToConstant: 146).isActive = true
        let cards = NSStackView()
        cards.orientation = .horizontal
        cards.alignment = .centerY
        cards.spacing = 10
        cards.edgeInsets = NSEdgeInsets(top: 2, left: 2, bottom: 5, right: 2)
        cards.translatesAutoresizingMaskIntoConstraints = false
        scroll.documentView = cards
        var buttons: [PieceSlotDropButton] = []
        for i in 0..<10 {
            let b = PieceSlotDropButton(title: "\(i + 1)\nLeer\n\nDatei hierher ziehen", target: self, action: #selector(pieceSlotPressed(_:)))
            b.tag = i
            ...
            b.heightAnchor.constraint(equalToConstant: 126).isActive = true
        ...
        cards.widthAnchor.constraint(greaterThanOrEqualToConstant: 1410).isActive = true
    ...
    private func buildV64BottomBar() -> NSView {
        ...
        playerProgress.widthAnchor.constraint(greaterThanOrEqualToConstant: 250).isActive = true
        ...
        playerVolume.widthAnchor.constraint(equalToConstant: 110).isActive = true
...
print('Applied V6.4 approved MusicChat layout: margins, chat+idea, ten large cards, bottom transport.')
